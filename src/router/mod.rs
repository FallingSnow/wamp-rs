...
use ws::{Builder as WSBuilder, Result as WSResult, Sender,
...
impl Router {
    ...
    pub fn new_with_ssl(ssl_acceptor: SslAcceptor) -> Router {
        Router {
            info: Arc::new(RouterInfo {
                realms: Mutex::new(HashMap::new()),
    ...
    pub fn listen(&self, url: &str) -> JoinHandle<()> {
        ...
        thread::spawn(move || {
            WSBuilder::new()
                .with_settings(WSSettings {
                    encrypt_server: router_info.ssl_acceptor.is_some(),
                    ..WSSettings::default()
                })
                .build(|sender| ConnectionHandler {
                    ...
                    ssl_acceptor: router_info.ssl_acceptor.clone(),
                })
                .unwrap()
                .listen(&url[..])
                .unwrap();
